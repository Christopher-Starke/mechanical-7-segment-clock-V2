...
void loop() {
  ...
  if (rtc.alarmFired(1)|| (millis()-watchdogTimer > watchdogTime)) { // if the rtc alarm flag is high update the time second function is there to identify if the rtcalarm function failed
    ...
    if (Hour < 21 && Hour > 7) { // used to disable the clock at night 
      offOn = true;      
      directMove();   
    } else if (offOn) {      
      timeArray[0] = 10;
    ...
    if (reading != ButtonState[i]) {
      ...
      if (lastButtonState[i] == LOW) {
        ...
        switch(i){
          case 0:
          timeArray[0] ++;
          if (timeArray[0]>9){ // rollover 
            timeArray[0] = 0;
          }           
          break;
          case 1:
            timeArray[1] ++;
          if (timeArray[1]>5){ // rollover 
            timeArray[1] = 0;
          }  
          break;
          case 2:
          timeArray[2] ++;
          if (timeArray[2]>9){ // rollover 
            timeArray[2] = 0;
          }            
          break;
          case 3:
            timeArray[3] ++;
          if (timeArray[3]>2){ // rollover 
            timeArray[3] = 0;
          }  
          break;          
        }
        ...
        DateTime newTime = {now.year(),now.month(),now.day(),Hour,Minutes,now.second()};
        rtc.adjust(newTime);
        directMove();
...
void sleepPower(){
  ...
  while(segmentPush < 28){ // send out next step over spi
    if ((sleepTimerNew - sleepTimerOld) < 50){
      byteOut = byteOut | stepOrder[currentStep[boardMap[segmentPush]]] << 4 * nibbleOutCount;
    }
    else if ((sleepTimerNew - sleepTimerOld) < 150){
      byteOut = 0b00000000;
    }    
    nibbleOutCount ++;

    if (nibbleOutCount == 2 ){  // collate steps into SPI byte
